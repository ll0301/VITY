...
namespace agora_gaming_rtc
{
	...
	public class IRtcEngine
	{
		...
		private static extern bool createEngine(string appId);
		[DllImport (MyLibName, CharSet = CharSet.Ansi)]
		...
		[DllImport (MyLibName, CharSet = CharSet.Ansi)]
		private static extern int enableLocalAudio (int enabled);
		[DllImport (MyLibName, CharSet = CharSet.Ansi)]
		...
		[DllImport (MyLibName, CharSet = CharSet.Ansi)]
		private static extern  int renewToken(string token);
		[DllImport (MyLibName, CharSet = CharSet.Ansi)]
		...
		[DllImport (MyLibName, CharSet = CharSet.Ansi)]
		private static extern  int setRemoteStreamDelayMS(int delayTime);
		...
		private static bool initEngine = false;

		private IRtcEngine(string appId)
		{
			initEngine = createEngine (appId);
		...
		public void Poll ()
		{
			...
			if (sArray [0].CompareTo ("onWarning") == 0) {
			...
			} else if (sArray [0].CompareTo ("onMicrophoneEnabled") == 0) {
				if (OnMicrophoneEnabled != null) {
					bool enabled = int.Parse (sArray [1]) != 0;
				...
				// TODO:
			} else if (sArray [0].CompareTo ("onRequestToken") == 0) {
        ...
        */
		...
		{
			return enableLocalAudio (enabled ? 1 : 0);
		}
		...
		public int SetRemoteStreamDelayMS (int delayTime)
		{
			return setRemoteStreamDelayMS (delayTime);
		}
